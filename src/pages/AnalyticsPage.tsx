...
const CHART_COLORS = ['#3D4D55', '#B58B63', '#A79E9C', '#C9C0B9', '#8b6843']
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl mx-auto space-y-8 py-2">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <Activity size={14} className="text-[#B58B63]" />
            <span className="text-[10px] font-mono tracking-widest text-[#B58B63] uppercase">TACTICAL ANALYSIS BOARD</span>
          </div>
          <h1 className="text-2xl font-display font-bold text-white tracking-tight mt-1">Credibility Analytics & Trends</h1>
          <p className="text-[#A79E9C] text-xs font-mono mt-1 uppercase">Real-time truth verification & network integrity monitoring</p>
        </div>

        {/* Date Picker Trigger */}
        <div className="flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded font-mono text-xs text-[#A79E9C] cursor-pointer hover:border-[#B58B63]/40 transition-colors">
          <Calendar size={13} className="text-[#B58B63]" />
          <span>{range}</span>
        </div>
      </div>
        ...
        {hudStats.map((stat, i) => (
          <Card key={i} className="border border-white/5 bg-[#1e272b]/30">
            <CardContent className="p-5">
              <div className="flex justify-between items-center text-[9px] font-mono text-[#A79E9C] uppercase tracking-wider mb-2">
              ...
              <div className="text-[9px] font-mono text-[#A79E9C] mt-2 uppercase tracking-wide opacity-60">
                {stat.detail}
              </div>
            </CardContent>
          </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <Card className="border border-white/10 bg-[#1e272b]/40 backdrop-blur-md">
                ...
                <AreaChart data={DAILY_VERIFICATIONS}>
                  <defs>
                    <linearGradient id="gCopper" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#B58B63" stopOpacity={0.25} />
                      <stop offset="95%" stopColor="#B58B63" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="2 2" stroke="rgba(255,255,255,0.03)" />
                  <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#A79E9C', fontFamily: 'monospace' }} stroke="rgba(255,255,255,0.05)" />
                  <YAxis tick={{ fontSize: 10, fill: '#A79E9C', fontFamily: 'monospace' }} stroke="rgba(255,255,255,0.05)" />
                  <Tooltip contentStyle={{ background: '#253035', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', fontSize: '11px', color: '#C9C0B9' }} />
                  <Area type="monotone" dataKey="verified" stroke="#B58B63" fill="url(#gCopper)" strokeWidth={2} name="Audits Run" />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
              ...
              <CardContent className="pt-6">
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    ...
                    <Tooltip contentStyle={{ background: '#253035', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', fontSize: '11px', color: '#C9C0B9' }} />
                    <Legend formatter={(value) => <span className="text-[10px] font-mono text-[#A79E9C] uppercase tracking-wider">{value}</span>} />
                  </PieChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <CardContent className="pt-6">
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={TOPIC_STATS} layout="vertical">
                    <CartesianGrid strokeDasharray="2 2" stroke="rgba(255,255,255,0.03)" />
                    <XAxis type="number" tick={{ fontSize: 10, fill: '#A79E9C', fontFamily: 'monospace' }} stroke="rgba(255,255,255,0.05)" />
                    <YAxis dataKey="topic" type="category" width={72} tick={{ fontSize: 10, fill: '#A79E9C', fontFamily: 'monospace' }} stroke="rgba(255,255,255,0.05)" />
                    <Tooltip contentStyle={{ background: '#253035', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', fontSize: '11px', color: '#C9C0B9' }} />
                    <Bar dataKey="count" fill="#B58B63" radius={[0, 2, 2, 0]} name="Audits" />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
        ...
        <div className="lg:col-span-4 h-full">
          <Card className="border border-white/10 bg-[#1e272b]/40 backdrop-blur-md h-full">
            <CardHeader className="border-b border-white/5 pb-4">
              <CardTitle className="text-white font-display text-sm flex items-center gap-2 uppercase tracking-wider font-semibold">
                ...
                return (
                  <div key={idx} className="flex gap-3 items-start text-xs font-mono">
                    ...
                    <div className="space-y-1 min-w-0">
                      <div className="text-[10px] text-[#A79E9C]">{log.time}</div>
