  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto space-y-10 py-6">
      ...
      <div className="text-center space-y-4">
        <div className="w-16 h-16 rounded-2xl bg-[#0F969C]/10 border border-[#0F969C]/30 flex items-center justify-center mx-auto shadow-[0_0_24px_rgba(15, 150, 156,0.15)]">
          <Shield size={28} className="text-[#0F969C]" />
        </div>
        <h1 className="text-3xl md:text-4xl font-display font-bold text-white tracking-tight">About Mis·Info</h1>
        <p className="text-[#6DA5C0] max-w-xl mx-auto text-sm leading-relaxed">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="border-white/10 bg-[#072e33]/40 backdrop-blur-sm">
          <CardContent className="p-6 space-y-4">
            <div className="w-10 h-10 rounded-lg bg-[#0F969C]/10 border border-[#0F969C]/30 flex items-center justify-center">
              <Target size={20} className="text-[#0F969C]" />
            </div>
            <h2 className="text-lg font-display font-semibold text-white">Our Mission</h2>
            <p className="text-sm text-[#6DA5C0] leading-relaxed">
        ...
        <Card className="border-white/10 bg-[#072e33]/40 backdrop-blur-sm">
          <CardContent className="p-6 space-y-4">
            <div className="w-10 h-10 rounded-lg bg-[#0F969C]/10 border border-[#0F969C]/30 flex items-center justify-center">
              <Brain size={20} className="text-[#0F969C]" />
            </div>
      ...
      <div className="space-y-6">
        <h2 className="text-xl font-display font-semibold text-white flex items-center gap-2">
          <Zap size={20} className="text-[#0F969C]" /> Core Processing Engine
        </h2>
        <div className="grid md:grid-cols-3 gap-4">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
              <Card className="border-white/10 bg-[#072e33]/30 hover:border-[#0F969C]/30 transition-all">
                <CardContent className="p-5 space-y-3">
                  <div className="w-9 h-9 rounded-lg bg-[#0F969C]/10 flex items-center justify-center">
                    <item.icon size={18} className="text-[#0F969C]" />
                  </div>
                  <h3 className="font-semibold text-white text-sm">{item.title}</h3>
                  <p className="text-xs text-[#6DA5C0] leading-relaxed">{item.desc}</p>
                </CardContent>
              </Card>
            </motion.div>
          ))}
      ...
      <div className="space-y-6">
        <h2 className="text-xl font-display font-semibold text-white flex items-center gap-2">
          <Code size={20} className="text-[#0F969C]" /> Engineering Architecture
        </h2>
        <div className="flex flex-wrap gap-2.5">
          {techStack.map((tech, i) => (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }}>
              <div className="flex items-center gap-2 px-4 py-2 rounded-full border border-white/5 bg-[#072e33]/40 backdrop-blur-sm">
                <span className="text-xs font-semibold text-white">{tech.name}</span>
                <Badge variant="outline" className="text-[9px] border-white/10 text-[#6DA5C0] tracking-wider uppercase font-mono">{tech.category}</Badge>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <div className="space-y-6">
        <h2 className="text-xl font-display font-semibold text-white flex items-center gap-2">
          <Rocket size={20} className="text-[#0F969C]" /> Development Roadmap
        </h2>
        <div className="grid md:grid-cols-2 gap-4">
          {roadmap.map((phase, i) => (
            <Card key={i} className={`border-white/10 bg-[#072e33]/40 backdrop-blur-sm ${phase.done ? 'border-[#0F969C]/30 shadow-[0_0_12px_rgba(15, 150, 156,0.06)]' : ''}`}>
              <CardContent className="p-5 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono text-[#6DA5C0] uppercase tracking-wider">{phase.phase}</span>
                ...
                <ul className="space-y-2">
                  {phase.items.map((item, j) => (
                    <li key={j} className="flex items-center gap-2 text-xs text-[#6DA5C0]">
                      <CheckCircle size={13} className={phase.done ? 'text-[#0F969C]' : 'text-[#6DA5C0]/30'} />
                      {item}
                    </li>
                  ))}
                </ul>
